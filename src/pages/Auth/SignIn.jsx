...
// Importing Hooks
// ================================
// import { useGoogleLogin } from "@react-oauth/google"; // ❌ تعطيل مؤقت
...
function SignIn() {
  ...
  // ================================
  // Function to sign in using Google account - مُعطل مؤقتاً
  // ================================
  const [errorGoolgeSignIn, setErrorGoolgeSignIn] = useState();
  
  // ✅ تعطيل Google OAuth مؤقتاً لحل المشكلة
  const handleGoogleSignIn = () => {
    setErrorGoolgeSignIn("Google Sign-In غير متاح حالياً. يرجى استخدام اسم المستخدم وكلمة المرور.");
  };

  // الكود الأصلي للـ Google OAuth (معطل)
  /*
  const signInWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        dispatch(signIn(storageUserInfo));
      } catch (err) {
        console.log(err);
    ...
    >
      <div className="px-5 space-y-10 w-full md:w-[650px] mx-auto lg:bg-white rounded-xl p-5 lg:dark:bg-gray-900">
        ...
        <div className="space-y-5">
          ...
          <h2 className="text-center text-xl text-white">مرحبا بعودتك!</h2>
        </div>

        {/* sign in with google - معطل مؤقتاً */}
        <div
          onClick={handleGoogleSignIn}
          className="bg-gray-200 dark:bg-white px-5 py-2 rounded-md h-[50px] overflow-hidden flex justify-between items-center cursor-pointer opacity-50"
        >
          <div className="">
          ...
          <div className="h-full w-fit ">
            <img className="w-full h-full" src={GoogleLogo} alt="" />
          </div>
        </div>
        {/* error sign in with google */}
        {errorGoolgeSignIn && (
          <span className="block text-red-500">{errorGoolgeSignIn}</span>
        )}
        ...
        <form className=" space-y-5" onSubmit={handleSubmit(onSubmit)}>
          ...
          <ButtonBasc title="تسجيل الدخول" className={"w-full block"} />
